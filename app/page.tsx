    ...
    <>
      <section className="hero">
        <div className="container">
          ...
          <div className="btn-row">
            ...
            <Link className="btn btn-secondary" href="/big-8">Explore the Big 8</Link>
          </div>
        </div>
      </section>
      <section className="section container">
        ...
        <div className="grid-3">
          {metals.slice(0, 6).map((m) => (
            <Link className="card" href={`/metals/${m.key}`} key={m.key}><h3>{m.label}</h3><p>Pathways, concerns, testing context, and practical reduction strategies.</p></Link>
          ))}
        </div>
      </section>
      <section className="section container">
        ...
        <div className="grid-3">
          {categories.slice(0, 6).map((c) => (
            <Link className="card" href={`/categories/${c.key}`} key={c.key}><h3>{c.label}</h3><p>How concentration, uptake, and processing shape risk patterns.</p></Link>
          ))}
        </div>
      </section>
      ...
      <section className="section container">
        ...
        <div className="news-grid">
          {latest.map((post) => (
            <article key={post.slug} className="news-card"><div className="img" /><div className="body"><p className="muted">{post.publishedAt}</p><h3><Link href={`/news/${post.slug}`}>{post.title}</Link></h3><p>{post.description}</p></div></article>
          ))}
        </div>
      </section>
      <section className="section container grid-3">
        <div className="card"><h3>How limits and testing work</h3><p>Learn why feasibility, toxicology thresholds, and method LOQ must align before a limit is operational.</p><Link href="/standards">Read standards explainer</Link></div>
